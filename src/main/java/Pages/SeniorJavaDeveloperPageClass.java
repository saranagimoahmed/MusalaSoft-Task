...
public class SeniorJavaDeveloperPageClass extends BaseClass {
    ...
    public boolean PageContainsValidation() {
        if (driver.getPageSource().contains("Requirements") && driver.getPageSource().contains("General description") && driver.getPageSource().contains("Responsibilities") && driver.getPageSource().contains("What we offer")) {
            return true;
        } else {
            return false;
        }
